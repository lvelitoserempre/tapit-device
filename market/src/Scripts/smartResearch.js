...
function goToTopPage() {// Override vtex scroll top function
...
jQuery.fn.vtexSmartResearch=function(opts)
{
    ...
    var defaults=
	{
		pageLimit: 1, // Número máximo de páginas que o script irá retornar. Exemplo "pageLimit=3" só será retornado resultados até a terceira página
	...
	var fn=
	{
		...
		{
			var url = '';
			var layout_id = $('li[layout]').attr('layout');

			var s=scroll||false;
			if(s){
				url = currentSearchUrl.replace(/PageNumber=[0-9]*/,"PageNumber="+currentPage);
			    if(url.includes(layout_id)){
					// url = url.replace('&sl='+layout_id, '');
				}
				return url;
			}
			else{
				url = (searchUrl+urlFilters).replace(/PageNumber=[0-9]*/,"PageNumber="+pageNumber);
				if(url.includes(layout_id)){
					// url = url.replace('&sl='+layout_id, '');
				}
				return url;
	...
	var fns=
	{
		...
		getPageCount: function(page_url){
			$.ajax({
				url: page_url,
				dataType: 'html',
				success: function(html) {
					var total_pages = html.search(/pagecount_/i);//search variable in string
					total_pages = html.substr(total_pages + 82, 30);//get variable string
					total_pages = total_pages.split('= ')[1];//get int and ;
					total_pages = total_pages.split(';')[0];//get just int
					total_pages = parseInt($.trim(total_pages));
					//console.log('total_pages: ', total_pages);
					fns.createPager(total_pages);
				}
			});
		},
		createPager: function(total_pages){
			var pager_id = $('.pager.bottom').attr('id').split('PagerBottom_')[1];

			$('.pager.bottom').empty();

			var PageClick = function(pageclickednumber) {
				window.location.hash = pageclickednumber;
				currentPage = pageclickednumber;//update page number in search url
				...
				$('#ResultItems_'+pager_id).load(currentSearchUrl, function(data) {
					$('#PagerBottom_'+pager_id).pager({ pagenumber: pageclickednumber, pagecount: total_pages, buttonClickCallback: PageClick });
					console.log(options.pagerCallback);
					options.pagerCallback();
				});
			}

			$('#PagerBottom_'+pager_id).pager({ pagenumber: 1, pagecount: total_pages, buttonClickCallback: PageClick });
			if (window.location.hash != '') PageClick(window.location.hash.replace(/\#/, ''));

		},
		addFilter:function(input)
		{
			window.location.hash = 1;
			currentPage = 1;//update page number in search url
			pageNumber = 1;
			...
			var page_url = currentSearchUrl.replace('/buscapagina', '/busca');
			fns.getPageCount(page_url);
